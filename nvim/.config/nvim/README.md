...
with first-class support for several languages and AI tooling added on top.

## What this config adds on top of upstream kickstart

### Language support

First-class LSP, formatting, and treesitter coverage for:

| Language | LSP | Formatter | Notes |
| --- | --- | --- | --- |
| Rust | `rust_analyzer` (with clippy on save) | `rustfmt` | `rustfmt`/`clippy` come from `rustup`, not Mason |
| TypeScript / JavaScript | `ts_ls` + `eslint` | `prettierd` → `prettier` | Covers `.ts`, `.tsx`, `.js`, `.jsx` (React & React Native) |
| Markdown | `marksman` | `prettierd` → `prettier` | |
| Python | `basedpyright` + `ruff` | `ruff_organize_imports` + `ruff_format` | |

All of the above **format on save**. LSPs and the `prettierd` formatter are
auto-installed via Mason on first launch; `rustfmt`/`clippy` must be added via
`rustup component add rustfmt clippy` (handled by the bootstrap script).

Extra treesitter parsers are installed: `rust`, `typescript`, `tsx`,
`javascript`, `jsdoc`, `python`, `json`, `css`, `toml`.

### AI assistants in a side split

Both assistants open as a toggleable right-side terminal pane (~40% width):

| Keymap | Action |
| --- | --- |
| `<leader>ac` | Toggle [Claude Code](https://github.com/coder/claudecode.nvim) |
| `<leader>af` | Focus Claude pane |
| `<leader>as` (visual) | Send selection to Claude |
| `<leader>ab` | Add current buffer as Claude context |
| `<leader>aa` / `<leader>ad` | Accept / deny a proposed Claude diff |
| `<leader>ar` / `<leader>aC` | Resume / continue last session |
| `<leader>am` | Switch Claude models |
| `<leader>kk` | Toggle Kiro CLI in a terminal split |

To leave the terminal split: `<Esc><Esc>` (or `<C-\><C-n>`) then `<C-w>h`.

Claude Code integration requires the `claude` CLI on `PATH`; Kiro requires
the `kiro` CLI. The bootstrap script warns if either is missing but does not
auto-install them.

### Layout additions

- `lua/custom/plugins/` — loaded automatically; drop additional plugin specs
  here (one file per plugin).
- `scripts/bootstrap.sh` / `scripts/bootstrap.ps1` — one-shot installers for
...
2. Run the bootstrap script for your OS (see next section).
3. Launch `nvim`. Lazy installs plugins, Mason installs LSPs/formatters, and
   treesitter compiles parsers — all on first run.

## Bootstrap scripts

Idempotent installers for every system dependency this config needs
(neovim, ripgrep, fd, git, node, python, rustup + rustfmt + clippy, a C
toolchain for `telescope-fzf-native`).
...
Supports Homebrew, apt, dnf, and pacman.
...
Both scripts are safe to re-run; they skip anything already installed and
print a warning (not an error) if the optional `claude` or `kiro` CLIs are
missing.

## Upstream kickstart documentation

Everything below is from upstream kickstart and still applies.
